...
export const PATCH = auth(async function PATCH(req, ctx) {
  ...
  if (participants.length > 0) {
    ...
    await Promise.allSettled(
      participants.map((p) =>
        resend.emails.send({
          from: "myloopclub <onboarding@resend.dev>",
          to: p.email,
          ...
          html: `<!DOCTYPE html><html lang="fr"><body style="margin:0;padding:0;background-color:#fffcf5;font-family:sans-serif;">
            <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#fffcf5;padding:40px 20px;">
              <tr><td align="center"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
                <tr><td style="padding-bottom:32px;text-align:center;"><span style="font-size:28px;font-weight:900;color:#0e59c3;">myloopclub</span></td></tr>
                ...
                <tr><td style="padding-top:24px;text-align:center;"><p style="margin:0;font-size:12px;color:rgba(0,0,0,0.3);">© ${new Date().getFullYear()} myloopclub.</p></td></tr>
              </table></td></tr>
            </table></body></html>`,
        })
      )
